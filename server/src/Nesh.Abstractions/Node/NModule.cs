...
{
    [Serializable]
    public delegate Task NEventCallback(INode node, Nuid id, INList args);
    ...
    public abstract class NModule
    {
        ...
        protected void RCEntity(string entity_type, EntityEvent entity_event, NEventCallback handler, int priority = 1)
        {
        ...
        protected void RCCustom(int custom_msg, NEventCallback handler, int priority = 1)
        {
            _CustomManager.Register(custom_msg, handler, priority);
        }

        protected void RCCommand(int command_msg, NEventCallback handler, int priority = 1)
        {
